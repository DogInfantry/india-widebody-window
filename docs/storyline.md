...
Answer first. Everything below supports the recommendation, nothing sets up to it.
...
*Every figure above is computed in-repo from DGCA, Eurostat and World Bank data by
`scripts/refresh.py`. None are typed by hand. See `data/data_dictionary.md` for source,
pull date and reliability grade on every field, and `docs/methodology.md` for what the data
cannot tell you.*
