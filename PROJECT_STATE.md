...
| 1 | Data pipeline + data dictionary + tests | Next | `pytest` green, Gulf share 2024 lands 50 to 53% |
...
1. **`jd.txt` is empty.** You are pasting the real Bain BCN AMS CoE posting. Blocks Phase 4 only.
...
3. **Repo name at push time.** Default `india-widebody-window`.
...
1. `src/data_pipeline.py` with the loader contract from the plan
2. Normalise two-digit years to four digits, filter the `Total Domestic` and
   `Total International` pseudo-airlines out of every carrier frame
3. Cache raw pulls to `data/raw/<source>_<YYYYMMDD>.csv`, write `data/raw/MANIFEST.md`
4. Write `data/data_dictionary.md` in this phase, not retrofitted later
5. `tests/test_pipeline.py` asserting the verified figures above
6. Commit
